...
if( ! class_exists( 'TIELABS_AJAX' ) ) {
	...
	class TIELABS_AJAX {
		...
		/**
		 * Block load-more may only include existing loop templates under templates/loops (no path injection).
		 *
		 * @param string $style Sanitized loop slug (e.g. from sanitize_title).
		 * @return bool
		 */
		private function is_allowed_block_loop_style( $style ) {

			if ( '' === $style || ! preg_match( '/^[a-z0-9-]+$/', $style ) ) {
				return false;
			}

			if ( 'woocommerce' === $style ) {
				return ( defined( 'TIELABS_WOOCOMMERCE_IS_ACTIVE' ) && TIELABS_WOOCOMMERCE_IS_ACTIVE );
			}

			return (bool) locate_template( "templates/loops/loop-{$style}.php", false, false );
		}
		...
		function block_load_more(){
			...
			if ( empty( $_REQUEST['block'] ) || ! is_array( $_REQUEST['block'] ) || empty( $_REQUEST['block']['style'] ) ) {
			...
			$block = wp_unslash( $_REQUEST['block'] );
			$style = sanitize_title( wp_unslash( $block['style'] ) );

			if ( ! $this->is_allowed_block_loop_style( $style ) ) {
				wp_send_json( wp_json_encode(
					array(
						'hide_next' => true,
						'hide_prev' => false,
			...
			if( ! empty( $_REQUEST['page'] ) ){
				$block['target_page'] = sanitize_text_field( wp_unslash( $_REQUEST['page'] ) );
			}

			// WooCommerce number of columns
			if( $style == 'woocommerce' && TIELABS_WOOCOMMERCE_IS_ACTIVE ){

				// Full Width section
				if( sanitize_text_field( $_REQUEST['width'] ) == 'full' ){
