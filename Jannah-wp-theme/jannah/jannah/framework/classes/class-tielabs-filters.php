...
class TIELABS_FILTERS {
		...
		if( $domain == TIELABS_TEXTDOMAIN || $domain == 'amp' ){
			...
			if( ! empty( $translations ) ){  // To minimize the calls of sanitize_title
				...
				if( ! empty( $translations[ $sanitize_text ] ) ){
					...
					$translated = htmlspecialchars_decode( $translations[ $sanitize_text ] );

					if( strpos( $text, '%s') !== false && strpos( $translated, '%s') === false ){
						return $text;
					}
	...
	function post_excerpt( $text = '' ){
		...
		if ( '' == $text ){
			...
			/**
			 * If images with class wp-image-{ID} exists in the conetnt, WordPress make a DB request to get and add the srcset
	...
	function dashboard_login_logo(){
		...
		if( tie_get_option( 'dashboard_logo' ) ){
			echo '
				<style type="text/css">
					.login h1 a {
						background-image:url('.tie_get_option( 'dashboard_logo' ).') !important;
