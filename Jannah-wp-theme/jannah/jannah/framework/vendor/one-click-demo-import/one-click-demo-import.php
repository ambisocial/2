	...
	<div id="tie-import-wrap" class="ocdi wrap about-wrap tie-about-wrap">
		...
		if( tie_get_token() && ! empty( $extra_demos ) && ! empty( $support_info['status'] ) && $support_info['status'] == 'expired' ){
			...
			<div class="tie-message-hint tie-message-success">
				<?php 
					printf(
						esc_html__( 'Your support period expired on %1$s. You can import any of the demos released before the end of your support period.', TIELABS_TEXTDOMAIN ),
		...
		?>
		<div class="" style="background: #FFF8DC; padding: 4% 4% 1%; overflow:hidden; border: 1px solid #ffeb8e; margin-top: 50px; border-radius: 8px; box-shadow: rgba(0, 0, 0, 0.1) 0px 25px 20px -20px;">
		...
		<?php
			...
			?>
				<div class="tie-message-hint">
					<?php
						printf(
							esc_html__( 'Your support period expired on %1$s. The demos listed below were released after the expiration of your support. To access and import these demos, you will need to renew your support subscription or purchase the %2$sTieLabs Golden Access%3$s.', TIELABS_TEXTDOMAIN ),
							'<strong>'. $support_info['human_date'] .'</strong>',
							'<a href="https://tielabs.com/go/jannah-golden-access-pass" target="_blank"><strong>',
							'</strong></a>'
						);
					?>
				</div>
