#!/usr/bin/env bash
# Taxonomia v2 — portal Estilos de vida, hobbies e consumo apaixonado
set -euo pipefail

REPO="${ESTRATO_REPO:-/var/www/estrato/repo}"
WP="${ESTRATO_WP:-/var/www/lifestyle.estrato.cc}"
PORTAL="${ESTRATO_PORTAL:-estrato-lifestyle}"

echo "=== Estrato Lifestyle: Taxonomia + RSS + keywords ==="

if [[ -f "$REPO/scripts/victor/validate-rss-feeds.py" ]]; then
  python3 "$REPO/scripts/victor/validate-rss-feeds.py" "$REPO/portals/estrato-lifestyle-taxonomy.php" || true
fi
...
if [[ -f "$REPO/scripts/victor/apply-portal-config.php" ]]; then
  ...
  sudo -u www-data wp --path="$WP" eval '
    if ( function_exists( "estrato_rss_apply_preset" ) ) {
      estrato_rss_apply_preset( "brasil-lifestyle" );
    }
  '
fi
...
echo "=== Taxonomia Lifestyle concluída ==="
echo "Fonte canônica: $REPO/portals/estrato-lifestyle-taxonomy.php"
echo "Preset RSS: brasil-lifestyle"
