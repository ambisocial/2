#!/usr/bin/env bash
# Taxonomia v2 — portal Sustentabilidade, economia alternativa e política de nicho
set -euo pipefail

REPO="${ESTRATO_REPO:-/var/www/estrato/repo}"
WP="${ESTRATO_WP:-/var/www/sustain.estrato.cc}"
PORTAL="${ESTRATO_PORTAL:-estrato-sustain}"

echo "=== Estrato Sustain: Taxonomia + RSS + keywords ==="

if [[ -f "$REPO/scripts/victor/validate-rss-feeds.py" ]]; then
  python3 "$REPO/scripts/victor/validate-rss-feeds.py" "$REPO/portals/estrato-sustain-taxonomy.php" || true
fi
...
if [[ -f "$REPO/scripts/victor/apply-portal-config.php" ]]; then
  ...
  sudo -u www-data wp --path="$WP" eval '
    if ( function_exists( "estrato_rss_apply_preset" ) ) {
      estrato_rss_apply_preset( "brasil-sustain" );
    }
  '
fi
...
echo "=== Taxonomia Sustain concluída ==="
echo "Fonte canônica: $REPO/portals/estrato-sustain-taxonomy.php"
echo "Preset RSS: brasil-sustain"
